...
def evaluate_point(
    ...
    """Evaluate all models at a single operating point."""
    models = _build_model_chain()
    composite = CompositeModel(models)

    speed_kts = speed_ms / KTS_TO_MS
    cruise_power_est = 0.3 * speed_ms + 0.5  # rough kW estimate
    ...
    conditions = {
        ...
        "density_altitude_ft": altitude_m * 3.281,
    }
    merged = dict(params)
...
def compute_envelope(
    ...
    """Compute the full operating envelope for a twin configuration."""
    t0 = time.time()
    params = _extract_params(twin)

    # Convert speed range to m/s (input may be in kts-equivalent)
    speeds = np.linspace(speed_range[0], speed_range[1], grid_resolution)
    ...
    for i, alt in enumerate(altitudes):
        for j, spd in enumerate(speeds):
            try:
                ...
                aero_ok = out.get("aero_feasible", 0.0)
                engine_ok = out.get("engine_feasible", 1.0)
                fuel_ok = out.get("fuel_feasible", 1.0)
                blur_ok = out.get("motion_blur_feasible", 1.0)
                z_feasible[i, j] = float(aero_ok and engine_ok and fuel_ok and blur_ok)
                z_ident[i, j] = out.get("identification_confidence", 0.0)
                z_endurance[i, j] = out.get("fuel_endurance_hr", 0.0)
            except Exception:
                z_feasible[i, j] = 0.0

    # UQ at representative mid-point
    ...
    uq_result = propagator.propagate(
        ...
        uq_inputs,
        output_names=[
            "fuel_endurance_hr", "identification_confidence",
    ...
    feasibility_surface = EnvelopeSurface(
        ...
        feasible_mask=[[bool(v) for v in row] for row in z_feasible.tolist()],
    )
    ...
    ident_surface = EnvelopeSurface(
        ...
        z_p5=(z_ident * 0.9).tolist(),
        z_p95=np.clip(z_ident * 1.05, 0, 1).tolist(),
    )
    ...
    response = EnvelopeResponse(
        ...
        battery_reserve=uq_result.outputs.get("fuel_endurance_hr"),
