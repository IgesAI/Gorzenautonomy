...
class IdentificationConfidenceModel(SubsystemModel):
    ...
    def parameter_names(self) -> list[str]:
        return [
            "accuracy_at_nominal",
            "accuracy_degradation_per_blur_px",
            "accuracy_degradation_per_jpeg_q10",
            "ood_threshold",
            "input_resolution_px",
        ]
    ...
    def evaluate(self, params: dict[str, float], conditions: dict[str, float]) -> ModelOutput:
        ...
        deg_per_blur = require_param(
            params, "accuracy_degradation_per_blur_px", "IdentificationConfidenceModel"
        )
        deg_per_q10 = require_param(
            params, "accuracy_degradation_per_jpeg_q10", "IdentificationConfidenceModel"
        )
        ...
        rs_distortion = require_param(
            conditions, "rs_total_distortion_px", "IdentificationConfidenceModel"
        )
        jpeg_q = require_param(conditions, "jpeg_quality", "IdentificationConfidenceModel")
        comp_qf = require_param(
            conditions, "compression_quality_factor", "IdentificationConfidenceModel"
        )
        pot = require_param(conditions, "pixels_on_target", "IdentificationConfidenceModel")
        latency_ms = require_param(
            conditions, "effective_latency_ms", "IdentificationConfidenceModel"
        )
        image_utility = require_param(
            conditions, "image_utility_score", "IdentificationConfidenceModel"
        )
        ...
        if pot >= PIXELS_EXCELLENT:
            ...
            pixel_density_factor = 0.5 + 0.5 * (pot - PIXELS_DETECT) / (
                PIXELS_EXCELLENT - PIXELS_DETECT
            )
